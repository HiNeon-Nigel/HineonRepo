...
function my_enqueue_script() {
    ...
    wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/css/bootstrap.min.css', array(), '1.0', 'all');
	 wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/css/accordion.scss', array(), '1.0', 'all');
	
	wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/css/accordion.css', array(), '1.0', 'all');
	...
	wp_enqueue_script( 'jquery', get_template_directory_uri().'/js/accordion.js', array(), '1.0', false);
	wp_enqueue_script( 'myjs', get_template_directory_uri().'/js/my.js', array(), '1.0', true);
...
function bbloomer_register_custom_order_status( $order_statuses ){
 ...
 foreach ( $order_statuses as $key => $status ) {
        ...
        if ( 'wc-pending' === $key ) {
            $new_order_statuses['wc-custom-status'] = _x( 'Paid', 'Order status', 'woocommerce' );  
        }
    }   
...
function bbloomer_show_custom_order_status( $order_statuses ) {      
   $order_statuses['wc-custom-status'] = _x( 'Paid', 'Order status', 'woocommerce' );       
   return $order_statuses;
}
*/
...
/*the order you want the status to be in. If you want to keep the position of a certain status just leave it blank. In this case the first status will retain its position*/
	...
	/*$key_order = array("","wc-pending","wc-custom-status","wc-processing","wc-on-hold","wc-completed","wc-cancelled","wc-refunded","wc-failed");
