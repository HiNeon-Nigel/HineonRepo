...
$hiscriptslt_text = get_field('cnsText', $post->ID);
$hiscriptslt_font = get_field('font', $post->ID);
$hiscriptslt_color= get_field('color', $post->ID);
$hiscriptslt_size = get_field('fSize', $post->ID);
...
$_product = wc_get_product( $thepostid );
  ...
  $accesories = get_nf_field('accesories');

  $costs = json_encode($sizes);
  $costs2 = json_encode($fcosts);
  $power = json_encode($power);
  ?>
  <div id="customV2" class="customizer open hiscriptslt">
    ...
    <input type="hidden" id="rowCost" value='<?= get_nf_field('row_cost') ?>'>
        ...
        <div class="cns-rows">
          <div class="tab-content">
            ...
            <div class="tab-pane active" id="t1">
              <div class="f-row" data-color="<?= sanitize_title($hiscriptslt_color)?>" data-font="<?= sanitize_title($hiscriptslt_font)?>" id="row-1">
                <div class="form-group">
                  <textarea type="text" rows="<?= $maxRows ?>" placeholder="Type your message" class="form-control input-text text-row" name="cnsText[]" id="cnsText" maxlength="<?= get_nf_field('max_characters_allowed_8in') ?>" pattern="[a-zA-Z0-9 -]+" title="Only letters and numbers are allowed." required><?php echo $hiscriptslt_text;?></textarea>
                </div>
                <div class="row">
                  <div class="col-lg-4">
                    <h4>Select Font</h4>
                    <div class="row font-picker">
                      ...
                      foreach ($font_group['adobe'] as $key => $font) {
                        ...
                        $active = ($font['name'] == $hiscriptslt_font)?"active":"";
                        echo '<div class="col-md-6 col-6"><div class="font '.$active.'" id="font-ae-'.($key+1).'" data-font="'.sanitize_title($font['name']).'">'.$font_firstWord.'</div></div>';
                      }
                      }
                       ?>
                       <?php
                       ...
                       foreach ($font_group['local'] as $key => $font) {
                         list($font_firstWord) = explode(' ', $font['name']);
                         $active = ($font['name'] == $hiscriptslt_font)?"active":"";
                         echo '<div class="col-md-6 col-6"><div class="font '.$active.'" id="font-lc-'.($key+1).'" data-font="'.sanitize_title($font['name']).'">'.$font_firstWord.'</div></div>';
                       }
                       }
                        ?>
                        <?php
                     ...
                     foreach ($font_group['google'] as $key => $font) {
                       ...
                       $active = ($font['name'] == $hiscriptslt_font)?"active":"";
                       echo '<div class="col-md-6 col-6"><div class="font '.$active.'" id="font-g-'.($key+1).'" data-font="'.sanitize_title($font['name']).'">'.$font_firstWord.'</div></div>';
                     }
                     }
                   }
                      ?>
                  ...
                  <div class="col-lg-4">
                    ...
                    <div class="colors">
                      <?php foreach ($colors as $key => $color):?>
                        <button class="color <?php if(sanitize_title($color['label']) == $hiscriptslt_color){ echo 'active'; } ?>" data-row="<?= $i ?>" data-color="<?= sanitize_title($color['label'])?>" type="button" style="background:<?= $color['color'] ?>" data-toggle="tooltip" title="<?= ucfirst(sanitize_title($color['label'])); ?>" data-placement="top">
                          <span ></span>
                        </button>
                      <?php endforeach; ?>
                    </div>
                  ...
                  <div class="col-lg-4">
                    ...
                    <div class="custom-select-wrapper">
                      <input type="hidden" class="select-row" value="<?= $hiscriptslt_size ?>">
                      <div class="row">
                        <?php foreach ($sizes as $key => $size): ?>
                          <div class="col-md-6">
                            <label class="custom-radio radioSize"><input type="radio" name="fSize" value="<?= $size['size'] ?>" <?php if($size['size'] == $hiscriptslt_size){ echo 'checked="checked"'; } ?> /> <span><?= str_replace('in', ' in', $size['size']) ?></span></label>
                          </div>
                        <?php endforeach; ?>
                      </div>
                    </div>
    ...
    <div class="container">
      ...
      <?php if($accesories): ?>
        <div class="accesories">
          <h3>Accesories</h3>
          <?php foreach ($accesories as $key => $accesorie): ?>
            <div class="accesorie">
              <div class="row">
                <div class="col-lg-6">
                  <figure><img src="<?= $accesorie['image']['sizes']['large'] ?>" alt="<?= $accesorie['title'] ?>"></figure>
                </div>
                <div class="col-lg-6">
                  <?php if($currency == 'USD'){
                    $cost = $accesorie['cost_us'];
                  }else{
                    $cost = $accesorie['cost'];
                  } ?>
                  <h3><?= $accesorie['title'] ?> <span><?= wc_price($cost) ?></span></h3>
                  <p><?= $accesorie['description'] ?></p>
                  <div class="buttons">
                    <button class="btn active btn-bg" data-val="no">No, Thanks</button>
                    <button class="btn btn-bg" data-val="yes">Buy <?= $accesorie['title'] ?> <?= wc_price($cost) ?></button>
                  </div>
                  <div class="radios d-none">
                    <label class="custom-radio"><input type="radio" name="accesorie-<?= $key ?>" data-name="<?= $accesorie['title'] ?>" data-cost="<?= $cost ?>" value="yes" /> <span>Yes</span></label>
          ...
          <div id="product-<?php the_ID(); ?>" <?php wc_product_class( '', $product ); ?>>
            ...
            remove_action( 'woocommerce_before_single_product_summary', 'wooswipe_woocommerce_show_product_thumbnails', 20 );
          	...
          	<div class="summary entry-summary">
          		...
          		add_action('woocommerce_single_product_summary', 'custom_price_ncs_2', 10);
