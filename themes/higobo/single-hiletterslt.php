...
<?php global $woocommerce , $post , $product;
  ...
  $hiletterslt_row_1 = get_field('row_1', $post->ID);
  $hiletterslt_row_2 = get_field('row_2', $post->ID);
  $hiletterslt_row_3 = get_field('row_3', $post->ID);
  $hiletterslt_color = get_field('color', $post->ID);
  $hiletterslt_imageID_1 = get_post_meta($post->ID, 'image_1', true);
  $hiletterslt_image_1 = wp_get_attachment_url($hiletterslt_imageID_1);
  $hiletterslt_imageID_2 = get_post_meta($post->ID, 'image_2', true);
  $hiletterslt_image_2 = wp_get_attachment_url($hiletterslt_imageID_2);
  $hiletterslt_imageID_3 = get_post_meta($post->ID, 'image_3', true);
  $hiletterslt_image_3 = wp_get_attachment_url($hiletterslt_imageID_3);
  ...
  <div id="neonCustomizer" class="images">
    ...
    <div id="customText" class="<?= sanitize_title($hiletterslt_color) ?> on"></div>
    <div id="customTextR" class="on">
      <div class="rows">
        <div class="in-row <?= sanitize_title($hiletterslt_color) ?>" data-color="<?= sanitize_title($hiletterslt_color) ?>" id="r-row-1"></div>
        <div class="in-row <?= sanitize_title($hiletterslt_color) ?>" data-color="<?= sanitize_title($hiletterslt_color) ?>" id="r-row-2"></div>
        <div class="in-row <?= sanitize_title($hiletterslt_color) ?>" data-color="<?= sanitize_title($hiletterslt_color) ?>" id="r-row-3"></div>
      </div>
  ...
  ?>
  <div id="customV1" class="customizer open hiletterlt">
            ...
            <div class="col-sm-8 col-md-9">
              <div class="tab-content">
                ...
                <div class="tab-pane <?php if($i == 1){ echo 'active'; } ?>" id="t<?= $i ?>">
                  <div class="f-row" data-color="<?= sanitize_title($hiletterslt_color)?>" id="row-<?= $i ?>">
                    <div class="form-group">
                      <textarea type="text" placeholder="Type your message" class="form-control input-text text-row" name="cnsText[]" id="cnsText" maxlength="<?= get_ns_field('max_characters_allowed_'.$sizes[0]['size']) ?>" pattern="[a-zA-Z0-9 -]+" title="Only letters and numbers are allowed." required><?php echo ${'hiletterslt_row_'.$i}; ?></textarea>
                    </div>
                    <div class="row">
                      <div class="col-lg-7">
                        <div class="col-lg-12">
                          <div class="row">
                            ...
                            <div class="col-lg-7">
                              <div class="colors">
                                <?php foreach ($colors as $key => $color): ?>
                                  <button class="color <?php if(sanitize_title($color['label']) == sanitize_title($hiletterslt_color)){ echo 'active'; } ?>" data-row="<?= $i ?>" data-color="<?= sanitize_title($color['label'])?>" type="button" style="background:<?= $color['color'] ?>" data-toggle="tooltip" title="<?= ucfirst(sanitize_title($color['label'])); ?>" data-placement="top">
                                    <span ></span>
                                  </button>
                                <?php endforeach; ?>
                              </div>
  ...
  <div class="customizer">
    <div class="container">
      ...
      <div class="llt-images">
        <div class="row">
          <?php if($hiletterslt_image_1): ?>
          <div class="col-lg-4">
            <img src="<?php echo $hiletterslt_image_1; ?>" >
          </div>
          <?php endif; ?>
          <?php if($hiletterslt_image_2): ?>
          <div class="col-lg-4">
            <img src="<?php echo $hiletterslt_image_2; ?>" >
          </div>
          <?php endif; ?>
          <?php if($hiletterslt_image_3): ?>
          <div class="col-lg-4">
            <img src="<?php echo $hiletterslt_image_3; ?>" >
          </div>
          <?php endif; ?>
        </div>
      </div>
