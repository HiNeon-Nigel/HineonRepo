...
function my_enqueue_script() {
    ...
    wp_enqueue_script( 'jquery', get_template_directory_uri().'/js/jquery.min.js', array(), '1.0', false);
    wp_enqueue_script( 'slick', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js', array(), '1.0', true);
    ...
    wp_localize_script( 'custom-js', 'jsobject', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
}
...
function get_custom_neon_signs_products() {
  if (defined( 'DOING_AJAX' ) && DOING_AJAX ){
    $args = array( 'post_type' => 'product', 'posts_per_page' => 9, 'product_cat' => 'custom-neon-signs', 'orderby' => 'id', 'order' => 'asc', 'offset' => 9 * $_POST['offset'] );
    ...
    $output = ob_get_contents();
    ob_end_clean();
    $args = array( 'post_type' => 'product', 'posts_per_page' => 9, 'product_cat' => 'custom-neon-signs', 'orderby' => 'id', 'order' => 'asc', 'offset' => 9 * ($_POST['offset'] + 1) );
    $loop = new WP_Query( $args );
    $next = false;
    if($loop->found_posts){
      $next = true;
    ...
    wp_reset_query();
    die(json_encode(array('data' => $output, 'next' => $next)));
  }
}
